...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 3. Algorithms Grid
        grp_algo = QGroupBox("3. Algorithms")
        layout_algo = QGridLayout()
        layout_algo.setSpacing(10)
        
        self.btn_aco = QPushButton("ACO\n(Ant Colony)")
        self.btn_aco.setFixedHeight(50)
        self.btn_aco.clicked.connect(self.run_aco)
        layout_algo.addWidget(self.btn_aco, 0, 0)
        
        self.btn_ilp = QPushButton("ILP\n(Baseline)")
        self.btn_ilp.setFixedHeight(50)
        self.btn_ilp.clicked.connect(self.run_ilp)
        layout_algo.addWidget(self.btn_ilp, 0, 1)
        
        self.btn_pareto = QPushButton("Pareto\n(Analysis)")
        self.btn_pareto.setFixedHeight(50)
        self.btn_pareto.clicked.connect(self.run_pareto)
        layout_algo.addWidget(self.btn_pareto, 1, 0, 1, 2) # Span 2 columns
        ...
        self.tabs.addTab(self.canvas_3d, "Pareto Analysis")
    ...
    def run_pareto(self):
        ...
        self.txt_output.append("Running Pareto Analysis...")
        analyzer = Pareto_Analyzer(self.network, s, d)
        
        self.worker = Worker(lambda: analyzer.run_analysis(50))
        self.worker.finished.connect(self.on_pareto_done)
        self.worker.start()
        
    def on_pareto_done(self, solutions):
        self.txt_output.append(f"Pareto Analysis: {len(solutions)} solutions.")
        ...
        if solutions:
            ...
            ax.set_xlabel('Delay', color='white')
            ax.set_ylabel('Reliability', color='white')
            ax.set_zlabel('Bandwidth', color='white')
